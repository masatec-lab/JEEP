        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 gap-12 lg:grid-cols-3">
            ...
            <div className="space-y-8">
              <div>
                <h3 className="text-sm font-semibold uppercase tracking-wider text-text-muted">Телефон</h3>
                <a
                ...
                <p className="mt-1 text-sm text-text-muted">{contacts.working_hours}</p>
              </div>

              <div>
                <h3 className="text-sm font-semibold uppercase tracking-wider text-text-muted">Мессенджеры</h3>
                <div className="mt-3 space-y-3">
                  ...
                  <a href={contacts.telegram} target="_blank" rel="noopener noreferrer"
                    ...
                    <div>
                      ...
                      <div className="text-xs text-text-muted">{contacts.telegram_username}</div>
                    </div>
                  </a>
              ...
              <div>
                <h3 className="text-sm font-semibold uppercase tracking-wider text-text-muted">Где мы находимся</h3>
                <p className="mt-3 text-sm text-text-secondary">{contacts.address}</p>
              </div>

              <div>
                <h3 className="text-sm font-semibold uppercase tracking-wider text-text-muted">Режим работы</h3>
                <p className="mt-3 text-sm text-text-secondary">{contacts.working_hours}</p>
              </div>
            </div>
            ...
            <div className="lg:col-span-2" id="booking">
              <div className="rounded-2xl border border-border bg-bg-secondary p-8 sm:p-10">
                <h2 className="text-2xl font-bold text-text-primary">Оставить заявку</h2>
                <p className="mt-2 text-sm text-text-muted">Заполните форму — мы перезвоним и подтвердим бронирование</p>
                ...
                {submitted ? (
                  <div className="mt-10 text-center py-12">
                    ...
                    <h3 className="mt-6 text-xl font-semibold text-text-primary">Заявка отправлена!</h3>
                    <p className="mt-2 text-text-secondary">Мы свяжемся с вами в течение 15 минут</p>
                    <button
                      onClick={() => { setSubmitted(false); setFormData({ name: "", phone: "", route: "", date: "", guests: "", message: "" }); }}
                  ...
                  <form onSubmit={handleSubmit} className="mt-8 space-y-6">
                    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                      <div>
                        <label htmlFor="name" className="block text-sm font-medium text-text-secondary">Ваше имя *</label>
                      ...
                      <div>
                        <label htmlFor="phone" className="block text-sm font-medium text-text-secondary">Телефон *</label>
                        <input type="tel" id="phone" name="phone" required value={formData.phone} onChange={handleChange}
                          className="mt-2 w-full rounded-lg border border-border bg-bg-primary py-3 px-4 text-sm text-text-primary placeholder:text-text-muted focus:border-accent focus:outline-none transition-colors"
                          placeholder="+7 (___) ___-__-__" />
                      </div>
                      <div>
                        <label htmlFor="route" className="block text-sm font-medium text-text-secondary">Маршрут</label>
                        <select id="route" name="route" value={formData.route} onChange={handleChange}
                      ...
                      <div className="sm:col-span-2">
                        <label htmlFor="guests" className="block text-sm font-medium text-text-secondary">Количество гостей</label>
                      ...
                      <div className="sm:col-span-2">
                        <label htmlFor="message" className="block text-sm font-medium text-text-secondary">Комментарий</label>
